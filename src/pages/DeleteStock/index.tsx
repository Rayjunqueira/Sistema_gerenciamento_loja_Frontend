    ...
    if (id) {
      try {
        const newStock = parseInt(stock) - parseInt(addStock);
          if (newStock < 0) {
            const zeroStock = 0;
            const updateProduct = await api.updateProduct(id, product, productCategoryName, productCategoryId, description, brand, zeroStock, parseFloat(totalCost), parseFloat(price));
          } else {
