...
const SingleOrder = () => {
  ...
  useEffect(() => {
    const getOrder = async (id: string | undefined) => {
      try { 
        if (id !== undefined) {
          const singleOrderInfo = await api.getOrderById(id);
  ...
  const handleRemoveOrder = async () => {
    try {
      const salesForDelete = await api.getSalesByOrder(id);
  
      if (salesForDelete.length === 0) {
        console.log("Sale not found!");
      } else {
        const lastSaleId = salesForDelete[salesForDelete.length - 1].sale_id;
        const deleteSale = await api.deleteSaleById(lastSaleId);
  
        if (deleteSale) {
          const deleteOrder = await api.deleteOrderById(id);
