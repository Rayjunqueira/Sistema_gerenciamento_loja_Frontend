...
const SingleCustomerCategory = (props: Props) => {
  ...
  useEffect(() => {
    const getCustomerCategory = async (id: string | undefined) => {
      try { 
        if (id !== undefined) {
          const customerCategoryInfo = await api.getCustomerCategoryById(id!);
  ...
  useEffect(() => {
    const getCustomersByCategory = async () => {
      try {
        const customers = await api.getCustomersByCategory(id);
  ...
  const handleDeleteCustomerCategory = async (id: string | undefined) => {
    if (id) {
      await api.deleteCustomerCategoryById(id);
      ...
      <div className="table-customerCategory">
        ...
        {customers.length === 0 && (
          <div>
            ...
            <button id="delete" onClick={() => handleDeleteCustomerCategory(customerCategoryId)}>Deletar</button>
          </div>
        )}
        </div>
