...
const AddCustomer = (props: Props) => {
  ...
  useEffect(() => {
    const getCustomerCategories = async () => {
      try {
        const customerCategoryList = await api.getAllCustomerCategories();
  ...
  const handleSaveCustomer = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (!customerCategoryId) {
      ...
      if (cellphone1 && customerCategoryId) {
        const customer = await api.createCustomer(name, customerCategoryId, email, cellphone1, cellphone2);
