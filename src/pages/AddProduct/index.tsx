...
const AddProduct = (props: Props) => {
  ...
  useEffect(() => {
    const getCategories = async () => {
      try {
        const productCategoryList = await api.getAllCategories();
  ...
  const handleSaveProduct = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    if (name && productCategoryId && totalCost && price && stock && brand) {
      const product = await api.createProduct(name, productCategoryId, description, brand, parseInt(stock), parseFloat(totalCost), parseFloat(price));
