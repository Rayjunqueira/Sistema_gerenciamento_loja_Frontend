    ...
    if (id) {
      try {
        const newDebt = parseFloat(totalDebt) - parseFloat(addDebt);
          if (newDebt < 0) {
            const zeroDebt = 0;
            const updateCustomer = await api.updateCustomer(id, customerName, customerCategoryId, customerEmail, customerCellphone1, customerCellphone2, zeroDebt);
          } else {
