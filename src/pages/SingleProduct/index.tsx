...
const SingleProduct = (props: Props) => {
  ...
  useEffect(() => {
    const getOrdersByProduct = async () => {
      try {
          const orders = await api.getOrdersByProduct(id);
  ...
  useEffect(() => {
    const getAllOrders = async () => {
      try {
        const allOrdersList = await api.getAllOrders();
    ...
    <div>
      <div className="table-container">
        ...
        <div className="removeEditContainer">
          <button id="edit" onClick={() => handleAddStock(productId)}>+ Estoque</button>
          <button id="edit" onClick={() => handleRemoveStock(productId)}>- Estoque</button>
          {orders.length === 0 && (
            <button id="delete" onClick={() => handleDeleteProduct(productId)}>Deletar</button>
          )}
        </div>
