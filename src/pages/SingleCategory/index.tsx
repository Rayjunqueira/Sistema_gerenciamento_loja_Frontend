...
const SingleProduct = (props: Props) => {
  ...
  useEffect(() => {
    const getCategory = async (id: string | undefined) => {
      try { 
        const categoryInfo = await api.getCategoryById(id);
  ...
  useEffect(() => {
    const getProductsByCategory = async (token: string | null) => {
      try {
        const products = await api.getProductsByCategory(id);
        ...
        <div className="removeEditContainer">
            {products.length === 0 && (
              <div>
                ...
                <button id="delete" onClick={() => handleDeleteCategory(categoryId)}>Deletar</button>
              </div>
            )}
          </div>
